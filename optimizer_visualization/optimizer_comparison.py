...
-----
    ...
    python optimizer_comparison.py --fake         # overlay fake histories (no CFD needed)
...
def _load_scores(experiment_id: str, dim: str) -> np.ndarray | None:
...
def plot_comparison(dim: str = "1d", include_fake: bool = False, log: bool = False, zoom: bool = False) -> None:
    runs = RUNS_1D if dim == "1d" else RUNS_2D
    fig, ax = plt.subplots(figsize=(10, 5))

    loaded: list[tuple[str, np.ndarray]] = []
    for exp_id, label in runs:
        scores = _load_scores(exp_id, dim)
        if scores is None:
    ...
    fig.tight_layout()
    suffix = "zoom" if zoom else ("log" if log else "linear")
    out_path = _PLOTS / f"optimizer_comparison_{dim}_{suffix}.png"
...
if __name__ == "__main__":
    ...
    plot_comparison(dim=args.dim, include_fake=args.fake, log=False)
    plot_comparison(dim=args.dim, include_fake=args.fake, log=True)
    plot_comparison(dim=args.dim, include_fake=args.fake, zoom=True)
