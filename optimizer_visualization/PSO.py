...
iteration spans three frames so every algorithm step reads apart:
  1) best update - personal/global best stars catch up with the evaluations
     of the previous move (the opening reveal frame plays this role for the
     initial evaluation),
  2) velocity - arrows from the current positions toward the next positions,
  3) move - particles at their new, freshly evaluated positions with the
     arrows removed; the best stars intentionally stay put until step 1 of
     the next iteration.
...
def main() -> None:
    ...
    def update_bests():
        """Fold the current evaluations into the personal/global bests."""
        nonlocal gbest_x, gbest_y
    ...
    # --- Frame loop ------------------------------------------------------ #
    # Each iteration is drawn as three frames so every algorithm step reads
    # apart:
    #   Step 1 (best update): the personal/global best stars catch up with the
    #                         evaluations of the previous move; positions
    #                         unchanged.
    #   Step 2 (velocity):    the current positions with arrows toward the
    #                         next positions; bests unchanged.
    #   Step 3 (move):        the particles at their new, freshly evaluated
    #                         positions with the arrows removed; the best
    #                         stars stay where they were until step 1 of the
    #                         next iteration.
    for t in range(N_ITERATIONS):
        draw = (t % PLOT_EVERY == 0) or (t == N_ITERATIONS - 1)
        show_pbest_now = (t <= PBEST_LAST_ITER)

        # Step 1 - update the personal and global bests from the evaluations
        # of the previous move.  At t == 0 the initialization already did this
        # and the opening reveal frame is exactly this step's picture, so
        # there is nothing new to compute or draw.
        if t > 0:
            update_bests()
            if draw:
                plot_frame(frame, positions, gbest_x, steps=None,
                           pbest_x=pbest_x, show_arrows=False,
                           show_pbest=show_pbest_now)
                frame += 1

        # Step 2 - determination of the next position (velocity arrows).
        ...
        # Step 3 - the move: apply the exact step that was drawn as arrows
        # and evaluate the new positions.  The best stars are deliberately
        # NOT refreshed here; they catch up in step 1 of the next iteration.
        positions  = next_pos
        velocities = next_vel
        values     = objective(positions[:, 0], positions[:, 1])
        if draw:
    ...
    # Closing frame: one last best update so the sequence ends with the final
    # personal/global bests on screen (and the printed result matches).
    update_bests()
    plot_frame(frame, positions, gbest_x, steps=None, pbest_x=pbest_x,
               show_arrows=False,
               show_pbest=(N_ITERATIONS - 1 <= PBEST_LAST_ITER))
